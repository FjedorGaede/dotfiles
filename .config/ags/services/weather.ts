...
>(
  ...
  return await Utils.fetch(addParametersToEndpoint(endpoint, parameters)).then(
    (res) => res.json(),
  );
};
...
geoLocation.connect("changed", ({ value: geoLoc }) => {
...
geoLocation.connect("changed", ({ value: geoLoc }) => {
  callEndpoint<CurrentWeatherRequestParameters, CurrentWeatherResponse>(
    "currentWeather",
    { lat: geoLoc.coordinates.lat, lon: geoLoc.coordinates.long },
  )
    .then((weather) => {
      const currWeather = convertResponseToCurrentWeather(weather);
      if (currWeather) {
        currentWeather.setValue(currWeather);
      }
    })
    .catch((err) => console.error(err));
});
